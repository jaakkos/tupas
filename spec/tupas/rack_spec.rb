  ...
  describe "Middleware" do
    ...
    describe "Redirect user depending to resonse" do
      it "Tupas provider responses with invalid message" do
        request = Rack::MockRequest.env_for("/_tupas/elisa-mobiilivarmenne-testi/132465/success?foo&bar&sfg")
      ...
      it "Tupas provider responses with message with invalid mac" do
        request = Rack::MockRequest.env_for("/_tupas/elisa-mobiilivarmenne-testi/132465/success?02K_VERS&B02K_TIMESTMP&B02K_IDNBR&B02K_STAMP&B02K_CUSTNAME&B02K_KEYVERS&03&B02K_CUSTID&08&B02K_CUSTTYPE&B02K_USERNAME&4")
        response = Tupas::Rack.new(@app).call(request)
        status(response).must_equal 301
      ...
      it "Tupas provider responses with message with out user info" do
        request = Rack::MockRequest.env_for("/_tupas/elisa-mobiilivarmenne-testi/132465/success?#{response_string(8, '00')}")
        response = Tupas::Rack.new(@app).call(request)
        status(response).must_equal 301
      ...
      it "Tupas provider responses with valid message" do
        request = Rack::MockRequest.env_for("/_tupas/elisa-mobiilivarmenne-testi/132465/success?#{response_string(8, '02')}")
        response = Tupas::Rack.new(@app).call(request)
        status(response).must_equal 301
      ...
      it "Tupas provider responses with invalid user info" do
        request = Rack::MockRequest.env_for("/_tupas/elisa-mobiilivarmenne-testi/132465/success?#{response_string(8, '11')}")
        response = Tupas::Rack.new(@app).call(request)
        status(response).must_equal 301
